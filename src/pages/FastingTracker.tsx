...
const FastingTracker = () => {
  ...
  const [fasts, setFasts] = useState(getActiveFasts());
  const [showPlans, setShowPlans] = useState(false);

  const handleStart = (plan: FastingPlan) => {
    startFast(plan);
    setFasts(getActiveFasts());
    setShowPlans(false);
  };

  const handleCheckin = (fastId: string) => {
    checkinFast(fastId);
    setFasts(getActiveFasts());
  };
      ...
      <div className="px-6 pt-6 animate-fade-in">
        ...
        {showPlans && (
          <div className="mb-6">
            ...
            <Button variant="outline" className="w-full mt-3 rounded-xl" onClick={() => setShowPlans(false)}>
              Cancel
            </Button>
          </div>
        )}
        ...
        {activeFasts.length === 0 && !showPlans && (
          <div className="text-center py-12">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Start a fasting plan to draw closer to God
            </p>
...
function FastCard({
  ...
  return (
    <div className="bg-card rounded-xl p-4 border border-border">
      <div className="flex items-start justify-between mb-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Day {fast.checkins.length} of {fast.durationDays} · Started {fast.startDate}
          </p>
        </div>
      ...
      <Button
        variant={checkedInToday ? "secondary" : "golden"}
        size="sm"
        className="w-full rounded-xl"
        onClick={() => onCheckin(fast.id)}
